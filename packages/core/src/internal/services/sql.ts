...
const Migrations = (config: GrepAiConfig) =>
  Migrator.fromRecord({
    ...
    '0003_create_chunk_embeddings_table': Effect.gen(function* () {
      ...
      const libsqlVecDimension = config.embedding.dimensions.toString()
        ...
        pg: () =>
          db`
            CREATE TABLE IF NOT EXISTS chunk_embeddings (
              ...
              , embedding  vector(${db.unsafe(pgVecDimension)}) NOT NULL
              , created_at TIMESTAMPTZ NOT NULL
            );
          `,
        orElse: () =>
          db`
            CREATE TABLE IF NOT EXISTS chunk_embeddings (
              id           INTEGER PRIMARY KEY
              , chunk_id   TEXT NOT NULL
              , embedding  F32_BLOB(${db.unsafe(libsqlVecDimension)}) NOT NULL
              , created_at TEXT NOT NULL
            );
          `,
      })
